...
function resolveIn(env) {
  const out = execFileSync(
    ...
    [
      "--input-type=module",
      "-e",
      `import { CONFIG_FILE, CLIENTS_FILE, LOG_FILE } from ${JSON.stringify(PATHS)};
       console.log(JSON.stringify({ CONFIG_FILE, CLIENTS_FILE, LOG_FILE }));`,
    ],
...
// An existing ~/.bagw wins over XDG — upgrades must not lose paired clients.
const legacyHome = mkdtempSync(join(tmpdir(), "bagw-legacy-"));
mkdirSync(join(legacyHome, ".bagw"));
p = resolveIn({ HOME: legacyHome, XDG_CONFIG_HOME: "/tmp/xdgcfg" });
assert.equal(p.CONFIG_FILE, join(legacyHome, ".bagw", "config.json"));
assert.equal(p.CLIENTS_FILE, join(legacyHome, ".bagw", "clients.json"));
